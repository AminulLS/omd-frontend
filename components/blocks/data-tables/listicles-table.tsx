  ...
  const columns: ColumnDef<Listicle>[] = [
    {
      ...
      cell: (row) => (
        <div className="size-12 rounded overflow-hidden bg-muted">
          ...
          {row.image ? <img src={row.image} alt={row.image_alt || row.title} className="size-full object-cover" /> : <div className="size-full flex items-center justify-center text-muted-foreground text-xs">No image</div>}
        </div>
      ),
    },
    {
    ...
    },
    {
      ...
      header: "Design Type",
      sortable: true,
      cell: (row) => <Badge variant={designTypeVariantMap[row.design_type]}>{designTypeLabelMap[row.design_type]}</Badge>,
    },
    {
      key: "created_at",
      header: "Created At",
    ...
    return (
      <Card>
        <CardContent className="p-6">
          <div className="text-center text-destructive">Error loading listicles: {error?.message || "Unknown error"}</div>
        </CardContent>
      </Card>
    );
  }
